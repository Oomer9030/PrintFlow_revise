...
class PlanningBoard(QWidget):
    ...
    def delete_row(self):
        ...
        for job in jobs_to_delete:
            if job in mc["jobs"]:
                jid = job.get("id")
                print(f"PLANNING BOARD: Deleting job {jid} (PJC: {job.get('pjc')})")
                mc["jobs"].remove(job)
                deleted_count += 1
                # SQL Delete: remove matching row from Production_Planner_Export
                sql_config = self.settings.get("sqlConfig") or self.settings.get("sql_config") or {}
                if sql_config and jid:
                    sql_service.delete_job_from_sql(str(jid), sql_config)
    ...
    def granular_save(self, job):
        """Pushes a single job change to SQL immediately."""
        if not self.settings.get("sqlExportEnabled"):
            print(f"GRANULAR SAVE: Skipping job {job.get('id')} because 'sqlExportEnabled' is False.")
            return
        ...
        # print(f"GRANULAR SAVE: Pushing job {job.get('id')} (PJC: {job.get('pjc')}) to SQL via thread...")
        # Run in a daemon thread to avoid UI lag
        threading.Thread(
            target=sql_service.save_single_job_to_sql,
            args=(job, self.current_machine, sql_config, user_name),
            daemon=True
        ).start()
    ...
    def apply_sql_result(self, sql_data, job):
        """Merges background SQL results into the job object."""
        ...
        if not real_job:
            api_service.log_to_file(f"DEBUG UI: Failed to find live UI reference for Job ID {target_id}")
            return
        ...
        if str(job.get("pjc")).strip() == str(sql_data.get("pjc")).strip():
            for key, val in sql_data.items():
                # Don't overwrite PJC itself, and only update if value is non-empty and not just a dash
                if key != "pjc" and val and str(val).strip() not in ["", "-"]:
                    # Do not overwrite 'pjcIn' with API data so we keep the system date
                    if key == "pjcIn":
                        continue
                        
                    current_val = str(job.get(key, "")).strip()
                    if current_val in ["", "-"] or current_val != str(val).strip():
                        api_service.log_to_file(f"PJC AUTO-FILL: Updating '{key}' to '{val}' for Job ID {job.get('id')}")
                        job[key] = val
                        updated = True
        
        if updated:
            api_service.log_to_file(f"PJC AUTO-FILL: SUCCESS. Refreshing table for PJC {sql_data.get('pjc')}")
            self.refresh_table()
            # PERSISTENCE FIX: Save the auto-filled data back to JSON and SQL
            self.save_data()
            self.granular_save(job)
        else:
            api_service.log_to_file(f"PJC AUTO-FILL: NO CHANGES applied for PJC {sql_data.get('pjc')}")
